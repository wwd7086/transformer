...
def get_batch(
    data_dir: str, split: str, batch_size: int, device: str = 'cpu'
...
class MNISTDataLoader:
    ...
    def __init__(self, data_dir: str, split: str, batch_size: int, device: str = 'cpu', shuffle: bool = True):
        """
        ...
        # Load data
        if split == "train":
            self.images = np.load(os.path.join(data_dir, "train_images.npy"), mmap_mode='r')
            self.labels = np.load(os.path.join(data_dir, "train_labels.npy"), mmap_mode='r')
        elif split == "test" or split == "val":
            self.images = np.load(os.path.join(data_dir, "test_images.npy"), mmap_mode='r')
            self.labels = np.load(os.path.join(data_dir, "test_labels.npy"), mmap_mode='r')
        else:
            raise ValueError(f"Invalid split: {split}. Must be 'train', 'test', or 'val'")
